...
_cmds.me = function(arr){
  ...
  if(arr1.length > 1){ //2 or 3
    ...
    if(_in && !_every && !_time && !_date){
      ...
      if(ret.valid === true){
        if(ret.type === "reldate"){
          create.createRelDate(ret, message);
        }
        else{
          create.createDate(ret, message);
        }
      }
      else{
        //NOT VALID
	    ...
	    if(ret1.valid === true){
		    ...
		    if(ret2.valid === true){
			    if(ret2.type === "reldate"){
				    create.createRelDateTime(ret1, ret2, message);
			    }
			    else{
				    create.createDateTime(ret1, ret2, message);
			    }
		    }
		    else{
			    //NOT VALID
		...
		var ret = time.isRelTime(arr1[arr2.indexOf("-i")]);
	    if(ret.valid === true){
	    	create.createRelTimeEvery(ret, message);
	    }
	    else{
	    	ret = time.isTime(arr1[arr2.indexOf("-t")]);
	    	if(ret.valid === true){
		    	create.createTimeEvery(ret, message);
	    	}
	    	else{
