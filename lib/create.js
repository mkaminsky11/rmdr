...
exports.createRelDateTime = function(_info, info, message){
    ...
    if(err){
      ...
      var job = crontab.create(exports.makeCmd(message,id),date,exports.makeComment(message, id));
...
exports.makeCmd = function(message, id){
  ...
  return 'wall -n '+ process.env[(process.platform == 'win32') ? 'USERPROFILE' : 'HOME'] +'/.rmdr/' + id + '.txt && rm -rf ' + process.env[(process.platform == 'win32') ? 'USERPROFILE' : 'HOME'] +'/.rmdr/' + id + '.txt'; //removes script
}
